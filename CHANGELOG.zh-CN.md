...
- 新增 Planning Contract V2：确定性来源清单、独立持久化的需求、验收标准、需求决策、计划快照，以及映射到验收项的实施和验证任务。
- 新增独立需求分析 Review、稳定的 Markdown/PDF 来源定位、受控交付角色与能力目录，并在 Service、HTTP API、CLI 和 Web 工作台中提供追加拆解与定向修订能力。

### 修复

- required 验收缺少实施或验证任务、高/关键需求决策未解决、没有合格执行者或缺少独立 Reviewer 时，审批统一失败关闭。
- 同来源重新拆解时冻结已解决 Decision 契约；模型返回过期、冲突或缺少来源的内容时明确拒绝，不再覆盖用户决策。
- Planning V2 部分写入失败时原子补偿，并拒绝并发拆解或定向修订产生的过期结果。

### 变更

- 新规划统一写入 Contract V2，历史计划继续兼容读取；规划覆盖与执行验证证据作为两类独立事实展示。
- 验证扩展为 216 个自动化测试，覆盖 `lscity-nuxt` 的 21 条验收/27 条待确认样本、追加/修订幂等、存储回滚、并发 stale、重启恢复、安装包以及桌面/移动端浏览器回归。
...
- 新增覆盖需求发现、需求评审、技术方案、任务拆解、开发实现、测试验证、代码评审和发布可靠性的 8 个默认智能体。
- 启动时按角色幂等补齐缺失智能体，同时保留已有用户智能体和历史默认智能体配置。
