...
- `PUT /projects/:id` 保存 Project metadata，不调用 AI，也不使当前计划失效。已有 Task 时，如果修改影响计划的字段（`cwd`、PRD、技术方案、优先级、负责人或任务语言），请求会以 `project-replan-required` 拒绝；名称和摘要仍可作为安全的纯 metadata 修改。没有 Task 的 draft 可以保留空 PRD，也可以继续提交空 PRD。
