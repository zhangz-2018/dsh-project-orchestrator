...
> 2026-08-25 补充校正：`1.5.11` 使用真实 `lscity-nuxt` PRD 拆解时发现，当前 Planner 输出契约仍以 Task 为中心，自动分解只创建一个 `bundle-root` RequirementItem，并由 Task 完成条件反向生成 AcceptanceCriterion；原始验收和待确认事项尚未逐条进入 Requirement/Decision 事实链。同时，Planner 生成的自由文本 requiredRoles/requiredCapabilities 与 Project Agent 的实际角色、能力声明不共享受控词汇，可能导致全部任务无合格候选。现有覆盖矩阵和候选门禁能失败关闭，但不能据此宣称需求拆解能力已经闭环。修复方案见 [任务拆解能力闭环修复设计](task-decomposition-capability-remediation-design.zh-CN.md)。以下内容描述已落地的基础设施能力，不再作为“任意真实 PRD 均能完整拆解”的验收结论。
