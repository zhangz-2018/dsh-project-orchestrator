...
function ProjectSquadBindingDrawer({ project, state, model, squadId }: { project: ProjectRecord; state: WorkbenchState; model: WorkbenchModel; squadId?: string }) {
  ...
  return <ManagementDrawer title="绑定 Squad" subtitle="建立项目团队关系，并补齐 Leader 与成员的参与资格。" close={model.closePanel} footer={<><ActionButton variant="ghost" onClick={model.closePanel}>取消</ActionButton><ActionButton variant="primary" data-testid="confirm-bind-squad" disabled={!selected || state.loading} onClick={() => void submit()}>绑定并同步</ActionButton></>}>
    ...
    {selected ? <section className="po-drawer-section" data-testid="squad-assignment-preview"><h3>绑定影响</h3><dl className="po-detail-facts"><dt>新增项目成员</dt><dd data-testid="squad-preview-additions">{additions.length === 0 ? '无需新增' : additions.map((id) => agentName(state.snapshot, id)).join('、')}</dd><dt>成员构成</dt><dd>{executable} 名可执行，{selected.memberAgentIds.length - executable} 名只读</dd><dt>任务分配</dt><dd>不修改现有 Task，绑定后可使用批量分配。</dd><dt>项目审批</dt><dd>已有审批会失效；初始组队不增加 revision。</dd></dl></section> : null}
...
function ProjectWorkspace({ project, state, model }: { project: ProjectRecord; state: WorkbenchState; model: WorkbenchModel }) {
  ...
  const currentReview = state.snapshot.projectReviews?.filter((review) => review.projectId === project.id && review.revision === project.revision).sort((left, right) => right.createdAt.localeCompare(left.createdAt))[0]
  const currentDelivery = state.snapshot.deliveryRecords?.find((record) => record.projectId === project.id && record.revision === project.revision)
  const reviewActor = project.owner?.trim() || '项目负责人'
  const confirmDelivery = () => void model.action(() => mutate(`/projects/${encodeURIComponent(project.id)}/delivery/confirm`, 'POST', { actor: reviewActor, note: '已确认本地交付包。' }), '交付已确认。')
  const closeDelivery = () => void model.action(() => mutate(`/projects/${encodeURIComponent(project.id)}/delivery/close`, 'POST', { actor: reviewActor, note: '本地交付包已接收并关闭。' }), '交付已关闭。')
  return (
...
function ProjectReviewResolutionPanel({ project, review, acceptanceCriteria, model, loading }: { project: ProjectRecord; review: ProjectReviewRecord; acceptanceCriteria: AcceptanceCriterionRecord[]; model: WorkbenchModel; loading: boolean }) {
  const [decision, setDecision] = useState<'approve' | 'request_changes' | 'reject' | 'waive'>('approve')
  const [actor, setActor] = useState(project.owner?.trim() || '项目负责人')
  const [note, setNote] = useState('')
  const [selectedAcceptanceIds, setSelectedAcceptanceIds] = useState<string[]>([])
  const [waiverReason, setWaiverReason] = useState('')
  const [waiverOwner, setWaiverOwner] = useState(project.owner?.trim() || '项目负责人')
  const [waiveIndependence, setWaiveIndependence] = useState(false)
  const [independenceRisk, setIndependenceRisk] = useState('')
  const [followUpAction, setFollowUpAction] = useState('')
  const incomplete = acceptanceCriteria.filter((criterion) => criterion.status !== 'verified' && criterion.status !== 'waived')
  const waiverReady = selectedAcceptanceIds.length > 0 && waiverReason.trim() !== '' && waiverOwner.trim() !== ''
  const independenceReady = waiveIndependence && waiverReason.trim() !== '' && waiverOwner.trim() !== '' && independenceRisk.trim() !== '' && followUpAction.trim() !== ''
  const canSubmit = actor.trim() !== '' && note.trim() !== '' && (decision !== 'waive' || waiverReady || independenceReady)
  const submit = async () => {
    const waivers = decision === 'waive' && waiverReady ? selectedAcceptanceIds.map((acceptanceId) => ({ acceptanceId, reason: waiverReason, owner: waiverOwner })) : []
    const reviewerIndependenceWaiver = decision === 'waive' && independenceReady ? { reason: waiverReason, owner: waiverOwner, risk: independenceRisk, followUpAction } : undefined
    await model.action(() => mutate(`/projects/${encodeURIComponent(project.id)}/review/resolve`, 'POST', { decision, actor, note, waivers, ...(reviewerIndependenceWaiver === undefined ? {} : { reviewerIndependenceWaiver }) }), decision === 'approve' ? '项目 Review 已批准。' : decision === 'waive' ? '项目 Review 已记录人工豁免。' : 'Review 已关闭并创建修订 Decision。')
  }
  return <section className="po-project-review-resolution" aria-label="项目 Review 决议"><div className="po-section-heading"><div><h3>项目 Review · 第 {review.round ?? 1} 轮</h3><p>{review.acceptanceResults?.filter((result) => result.result === 'passed').length ?? 0}/{review.acceptanceResults?.length ?? acceptanceCriteria.length} 个验收结果通过</p></div></div><div className="po-review-fields"><label><span>决议</span><select className="po-select" value={decision} onChange={(event) => setDecision(event.target.value as typeof decision)}><option value="approve">批准</option><option value="request_changes">要求修改</option><option value="reject">拒绝</option><option value="waive">人工豁免</option></select></label><label><span>责任人</span><input className="po-input" value={actor} onChange={(event) => setActor(event.target.value)} /></label><label className="po-review-note"><span>Review 记录</span><textarea className="po-textarea" value={note} onChange={(event) => setNote(event.target.value)} /></label></div>{decision === 'waive' ? <div className="po-review-waivers">{incomplete.length > 0 ? <fieldset><legend>验收项豁免</legend>{incomplete.map((criterion) => <label className="po-check" key={criterion.id}><input type="checkbox" checked={selectedAcceptanceIds.includes(criterion.id)} onChange={(event) => setSelectedAcceptanceIds((current) => event.target.checked ? [...current, criterion.id] : current.filter((id) => id !== criterion.id))} />{criterion.statement}</label>)}</fieldset> : null}<label className="po-check"><input type="checkbox" checked={waiveIndependence} onChange={(event) => setWaiveIndependence(event.target.checked)} />豁免 Reviewer 独立性</label><div className="po-review-fields"><label><span>豁免原因</span><textarea className="po-textarea" value={waiverReason} onChange={(event) => setWaiverReason(event.target.value)} /></label><label><span>风险责任人</span><input className="po-input" value={waiverOwner} onChange={(event) => setWaiverOwner(event.target.value)} /></label>{waiveIndependence ? <><label><span>风险</span><textarea className="po-textarea" value={independenceRisk} onChange={(event) => setIndependenceRisk(event.target.value)} /></label><label><span>后续动作</span><textarea className="po-textarea" value={followUpAction} onChange={(event) => setFollowUpAction(event.target.value)} /></label></> : null}</div></div> : null}<div className="po-gate-actions"><ActionButton variant={decision === 'reject' ? 'outline' : 'primary'} icon={decision === 'approve' ? <IconCheckOutline16 /> : undefined} disabled={loading || !canSubmit} onClick={() => void submit()}>提交 Review 决议</ActionButton></div></section>
}

function DeliveryResponsibilitySummary({ delivery, review, snapshot }: { delivery: NonNullable<Snapshot['deliveryRecords']>[number]; review?: ProjectReviewRecord | undefined; snapshot: Snapshot }) {
  const chain = delivery.responsibilityChain!
  return <details className="po-artifact-disclosure"><summary>交付责任链（{chain.tasks.length} 个 Task · {chain.delegations.length} 次 Delegation）</summary><div className="po-responsibility-summary"><div><strong>规划与审阅</strong><span>Planner：{agentName(snapshot, chain.plannerAgentId)} · Lead：{agentName(snapshot, chain.leadAgentId)} · Reviewer：{review?.reviewerId ?? agentName(snapshot, chain.plannedReviewerAgentId)}</span></div>{chain.tasks.map((task) => <div key={task.taskId}><strong>{taskName(snapshot, task.taskId)}</strong><span>{agentName(snapshot, task.ownerAgentId)} · {task.assignmentMode === 'squad_delegation' ? 'Squad 协作' : '单 Agent'} · {task.taskRunIds.length} 次 TaskRun · {task.artifactIds.length + task.verificationEvidenceIds.length} 项证据{task.wasReassigned ? ' · 已重新分派' : ''}</span></div>)}{chain.delegations.map((delegation) => <div key={delegation.delegationId}><strong>Delegation {delegation.delegationId.slice(0, 8)}</strong><span>{agentName(snapshot, delegation.leaderAgentId)} → {agentName(snapshot, delegation.memberAgentId)} · {delegation.evidenceIds.length} 项证据</span></div>)}<small>{chain.reviewIds.length} 个 Review · {chain.decisionIds.length} 个 Decision · {chain.activityIds.length} 条 Activity</small></div></details>
}
...
function TeamPlanPanel({ project, state }: { project: ProjectRecord; state: WorkbenchState }) {
  const [plan, setPlan] = useState<ProjectTeamPlan>()
  const [error, setError] = useState<string>()
  useEffect(() => { setPlan(undefined); setError(undefined); void readApi<ProjectTeamPlan>(`/projects/${encodeURIComponent(project.id)}/team-plan`).then(setPlan).catch((reason) => setError(messageOf(reason))) }, [project.id, project.revision])
  if (error) return <section className="po-team-plan-panel"><div className="po-inline-error"><IconWarningOutline16 />团队计划读取失败：{error}</div></section>
  if (plan === undefined) return <section className="po-team-plan-panel"><p>正在计算团队资格、关键路径和等待原因…</p></section>
  const { preflight } = plan
  const taskNames = new Map(plan.tasks.map((task) => [task.id, task.title]))
  const roleAgentName = (agentId: string | undefined) => agentId === undefined ? '未配置' : agentName(state.snapshot, agentId)
  const roleOwners = (kind: TaskRecord['kind']) => [...new Set(plan.tasks.filter((task) => task.kind === kind).map((task) => agentName(state.snapshot, task.agentId)))].join('、') || '未配置'
  return <section className="po-team-plan-panel" aria-label="团队计划预检">
...
function TaskCandidateInspector({ project, task, state, model }: { project: ProjectRecord; task: TaskRecord; state: WorkbenchState; model: WorkbenchModel }) {
  ...
  const [candidates, setCandidates] = useState<ProjectTaskCandidates['candidates']>([])
  const [squadCandidates, setSquadCandidates] = useState<ProjectTaskCandidates['squadCandidates']>([])
  const [conflicts, setConflicts] = useState<string[]>([])
  const [loaded, setLoaded] = useState(false)
  const [selected, setSelected] = useState('')
  useEffect(() => { if (!open) return; setLoaded(false); void readApi<ProjectTaskCandidates>(`/projects/${encodeURIComponent(project.id)}/agent-candidates?taskId=${encodeURIComponent(task.id)}`).then((result) => { setCandidates(result.candidates); setSquadCandidates(result.squadCandidates); setConflicts(result.conflicts); setLoaded(true) }).catch(() => { setCandidates([]); setSquadCandidates([]); setConflicts([]); setLoaded(true) }) }, [open, project.id, task.id, project.revision])
  const eligible = candidates.filter((candidate) => candidate.eligible)
  const reassign = async () => { if (!selected) return; const result = await model.action(() => mutate(`/projects/${project.id}/reassign-task`, 'POST', { expectedRevision: project.revision, taskId: task.id, agentId: selected, actor: 'Harness user' }), '任务已显式改派，审批已失效。'); if (result) setOpen(false) }
  const recordBlocker = async () => { const reason = `没有可执行的团队候选。${conflicts.length > 0 ? `冲突键：${conflicts.join('、')}。` : ''}`; await model.action(() => mutate(`/projects/${project.id}/resolve-team-blocker`, 'POST', { taskId: task.id, reason, facts: [...candidates.filter((candidate) => !candidate.eligible).slice(0, 6).map((candidate) => `Agent ${candidate.agentId}: ${candidate.reasons.join(',')}`), ...squadCandidates.filter((candidate) => !candidate.eligible).slice(0, 6).map((candidate) => `Squad ${candidate.squadId}: ${candidate.reasons.join(',')}`)], actor: 'Harness user' }), '团队阻塞已记录到 Inbox/Decision。') }
  return <details className="po-task-candidates" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}><summary>候选与原因</summary>{!loaded ? <p>正在读取统一候选校验…</p> : <div><p>{eligible.length} 个 Agent 候选可执行，{candidates.length - eligible.length} 个被拒绝。</p>{task.agentId ? <p>当前 owner：{agentName(state.snapshot, task.agentId)}</p> : null}<select className="po-select" aria-label={`${task.title} 的改派候选`} value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">选择可执行候选</option>{eligible.map((candidate) => <option key={candidate.agentId} value={candidate.agentId}>{agentName(state.snapshot, candidate.agentId)} · {candidate.runtimeStatus} · 可用槽位 {candidate.availableSlots}</option>)}</select><ActionButton size="sm" variant="outline" disabled={!selected || selected === task.agentId || state.loading} onClick={() => void reassign()}>显式改派</ActionButton>{eligible.length === 0 ? <ActionButton size="sm" variant="ghost" disabled={state.loading} onClick={() => void recordBlocker()}>记录团队阻塞</ActionButton> : null}{conflicts.length > 0 ? <small>冲突键：{conflicts.join('、')}</small> : null}<ul>{candidates.filter((candidate) => !candidate.eligible).slice(0, 6).map((candidate) => <li key={candidate.agentId}>{agentName(state.snapshot, candidate.agentId)}：{candidate.reasons.join('、')}</li>)}</ul>{task.assignmentPolicy?.mode === 'squad_delegation' ? <div><strong>Squad 候选</strong><p>{squadCandidates.filter((candidate) => candidate.eligible).length} 个可派发，{squadCandidates.filter((candidate) => !candidate.eligible).length} 个不可用。</p><ul>{squadCandidates.map((candidate) => <li key={candidate.squadId}>{state.snapshot.squads.find((squad) => squad.id === candidate.squadId)?.name ?? candidate.squadId}：{candidate.eligible ? `可派发 · ${candidate.availableSlots} 个空闲槽位` : candidate.reasons.join('、')}</li>)}</ul></div> : null}</div>}</details>
}
...
function ProjectSquadBindingsSection({ project, state, model }: { project: ProjectRecord; state: WorkbenchState; model: WorkbenchModel }) {
  ...
  const [impact, setImpact] = useState<ProjectTeamImpact>()
  useEffect(() => { void readApi<ProjectTeamImpact>(`/projects/${encodeURIComponent(project.id)}/team-impact`).then(setImpact).catch(() => setImpact(undefined)) }, [project.id, project.revision, state.snapshot.delegations, state.snapshot.issues, state.snapshot.tasks, state.snapshot.projectSquadBindings])
...
function TaskDialog({ state, model, task }: { state: WorkbenchState; model: WorkbenchModel; task?: TaskRecord | undefined }) {
  ...
  const [riskLevel, setRiskLevel] = useState<TaskRiskLevel>(task?.assignmentPolicy?.riskLevel ?? 'low')
  ...
  const save = async () => {
    ...
    const assignmentPolicy = task?.assignmentPolicy === undefined
      ? { mode: 'single_agent' as const, riskLevel, requiredRoles: [], requiredCapabilities: [], allowedAgentIds: [], allowedSquadIds: [], requiresIndependentReviewer: false, maxParallel: 1, conflictKeys: [], allowedScope: [], forbiddenScope: [], escalationConditions: [] }
      : { ...task.assignmentPolicy, riskLevel }
    const payload = { kind, priority, tags: taskTags, title, description, acceptanceCriteria, dependencies, agentId: agentId || null, assignmentPolicy, testCommand }
    const result = await model.action(() => task === undefined ? mutate<TaskRecord>(`/projects/${projectId}/tasks`, 'POST', payload) : mutate<TaskRecord>(`/tasks/${task.id}`, 'PUT', { priority, tags: taskTags, title, description, acceptanceCriteria, dependencies, agentId: agentId || null, assignmentPolicy, testCommand }), task === undefined ? '任务已添加到项目计划。' : '任务已更新，项目需要重新批准。')
    if (result) model.closePanel()
  }
  const remove = async () => {
